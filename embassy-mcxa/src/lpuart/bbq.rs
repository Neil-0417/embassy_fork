...
impl BbqState {
    ...
    unsafe fn start_read_transfer(&'static self, info: &'static Info) -> bool {
        ...
        if (state & STATE_RXDMA_MODE_CONTINUOUS) != 0 {
            // SAFETY: The function-level requirements also satisfy continuous RX startup.
            return unsafe { self.start_continuous_read(info) };
        }
